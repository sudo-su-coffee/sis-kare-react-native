...
Android builds need an SDK path. Copy `android/local.properties.example` to `android/local.properties` and set `sdk.dir` to the local Android SDK path. The local properties file is intentionally not committed because it is machine-specific.
...
The client does not bundle credentials, access tokens, or real student data. Its Android manifest disables clear-text traffic, backups, and user-added certificate trust. The session helper uses `react-native-keychain` so a production sign-in can keep short-lived session material in the operating system keychain or keystore. The SIS backend must still enforce identity, authorization, record ownership, input validation, audit logging, rate limiting, and HTTPS for every request.
